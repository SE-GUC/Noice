...
const functions = {
    ...
    cancelMyApplication: async(id,body)=>{
        const vacancy = await axios.put(`http://localhost:5000/api/vacancy/apply/cancelMyApplication/${id}`,body)
        return vacancy
        
    },
    closeVacancy: async(id)=>{
        const vacancy = await axios.put(`http://localhost:5000/api/vacancy/apply/closeVacancy/${id}`)
        return vacancy
    },
