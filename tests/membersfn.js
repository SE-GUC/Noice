...
const functions = {
    viewMember : async () =>{
        const members = await axios.get('http://localhost:3000/api/users/member/')
        return members
    },

    viewMemberId : async(id) =>{
        const member = await axios.get(`http://localhost:3000/api/users/member/${id}`)
        return member
    },
    updateMember : async(id,req) =>{
        const member = await axios.put(`http://localhost:3000/api/users/member/${id}`, req)
        return member
    },
    createMember : async(req) =>{
        const member = await axios.post('http://localhost:3000/api/users/member/', req)
        return member
    },
    deleteMember : async(id) =>{
        const member = await axios.delete(`http://localhost:3000/api/users/member/${id}`)
        return member
    },
    //admin stuff
    ...
    viewLocationByMember : async () =>{
        const members = await axios.get('http://localhost:3000/api/users/member/location/')
        return members
    },
    //view members through location
    viewMemberByLocation : async () =>{
        const members = await axios.get('http://localhost:3000/api/users/location/member/')
        return members
    }
}
