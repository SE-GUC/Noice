...
//{Redux}
import {updateRoomRes} from '../../actions/viewReservationsFromLocView/resActions'
import {connect} from 'react-redux';
//import prop types which validates the inputs to this components
import PropTypes from 'prop-types';
// 

class UpdateRoomRes extends Component {
    constructor(props){
      super(props)
    if(props.id){
    ...
    componentDidMount(){
      ...
      console.log("the triggered reservation start data edit is: " + this.props.match.params.id)
      const d = new Date(Number(this.props.match.params.id))
      console.log("the triggered reservation start data edit is: " + d)
      const startDateFor = moment(d).format('DD-MM-YYYY hh:mm')
      console.log("the triggered reservation start data edit is: " + startDateFor)
      ...
      this.props.reservations.forEach(element=>{
        if(element.startDate==startDateFor){
          oelement=element
        }
      })
      console.log("the element is after selection: ")
      console.log(oelement)
      this.setState({
        reserverId:oelement.reserverId,
        startDate: new Date(oelement.startDate),
        endDate: new Date(oelement.endDate)
      })
    }
    handleGoBackButton = () =>{
    ...
    handleSubmit = () =>{
      ...
      console.log(body)
      this.props.updateRoomRes(id,body)
      // axios request
    }  
  render() {
            ...
            <Grid container  direction="column" alignItems="stretch" className ={classes.grid}>
                  ...
                  <Grid container direction="row" >
                    ...
                    <Grid style={{flexGrow:1}}item> 
                      ...
                      <Typography className = {classes.typStyle} variant="headline">Update Room Reservations</Typography>
                    </Grid>
                  ...
                  <Button style={{margin:'auto'}} onClick={this.handleSubmit}variant="contained" color="primary" className={classes.button}>Update room reservation</Button>        
            </Grid>
            </Paper>
        </div>
    )
...
const styles = theme =>({
  ...
  UpdateRoomRes.propTypes ={
    updateRoomRes: PropTypes.func.isRequired
  };
  const mapStateToProps = state =>({
    reservations: state.roomResLoc.reservations
  })

  export default connect(mapStateToProps,{updateRoomRes})(withStyles(styles)(UpdateRoomRes))